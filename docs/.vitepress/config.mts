import { defineConfig } from 'vitepress'

export default defineConfig({
  base: '/',
  title: 'Laravel AI Evaluation',
  description: 'Real-call LLM evals for Laravel AI',
  head: [['link', { rel: 'icon', href: '/box.svg' }]],
  themeConfig: {
    logo: '/box.svg',
    search: {
      provider: 'local',
    },
    nav: [
      { text: 'Home', link: '/' },
      { text: 'First Eval', link: '/first-eval' },
      {
        text: 'Pest',
        items: [{ text: 'Run in Pest', link: '/running-in-pest' }],
      },
      {
        text: 'Standalone',
        items: [{ text: 'Run standalone', link: '/running-standalone' }],
      },
      {
        text: 'Expectations',
        items: [
          { text: 'Overview', link: '/expectations' },
          { text: 'Deterministic expectations', link: '/deterministic-expectations' },
          { text: 'LLM-as-judge expectations', link: '/llm-as-judge-expectations' },
          { text: 'Custom expectations', link: '/custom-expectations' },
          { text: 'Dataset evals', link: '/datasets' },
          { text: 'Conversation evals', link: '/conversation-evals' },
        ],
      },
      {
        text: 'Guides',
        items: [
          { text: 'Installation', link: '/installation' },
          { text: 'Concepts', link: '/concepts' },
          { text: 'Create eval files', link: '/creating-evals' },
          { text: 'Dataset evals', link: '/datasets' },
          { text: 'Conversation evals', link: '/conversation-evals' },
          { text: 'API reference', link: '/api-reference' },
          { text: 'Troubleshooting', link: '/troubleshooting' },
          { text: 'Dealing with rate limits', link: '/dealing-with-rate-limits' },
          { text: 'When to run evals', link: '/when-to-run-evals' },
          { text: 'Run in CI', link: '/running-in-ci' },
        ],
      },
    ],
    socialLinks: [
      {
        icon: {
          svg: '<svg class="w-full h-full" fill="currentColor" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 32 32">\n' +
              '<path fill="currentColor" d="M16,0C7.16,0,0,7.34,0,16.41c0,7.25,4.58,13.4,10.94,15.57c0.8,0.15,1.09-0.36,1.09-0.79\n' +
              '\tc0-0.39-0.01-1.42-0.02-2.79c-4.45,0.99-5.39-2.2-5.39-2.2c-0.73-1.9-1.78-2.4-1.78-2.4c-1.45-1.02,0.11-1,0.11-1\n' +
              '\tc1.61,0.12,2.45,1.69,2.45,1.69c1.43,2.51,3.74,1.78,4.66,1.36c0.15-1.06,0.56-1.78,1.02-2.19c-3.55-0.41-7.29-1.82-7.29-8.11\n' +
              '\tc0-1.79,0.62-3.26,1.65-4.4C7.27,10.73,6.72,9.06,7.6,6.8c0,0,1.34-0.44,4.4,1.68c1.28-0.36,2.64-0.55,4.01-0.55\n' +
              '\tc1.36,0.01,2.73,0.19,4.01,0.55c3.05-2.12,4.4-1.68,4.4-1.68c0.87,2.26,0.32,3.93,0.16,4.34c1.03,1.15,1.64,2.61,1.64,4.4\n' +
              '\tc0,6.3-3.74,7.69-7.31,8.1c0.57,0.51,1.09,1.51,1.09,3.04c0,2.19-0.02,3.96-0.02,4.5c0,0.44,0.29,0.95,1.1,0.79\n' +
              '\tC27.42,29.8,32,23.65,32,16.41C32,7.34,24.84,0,16,0z"></path>\n' +
              '</svg>\n',
        },
        link: 'https://github.com/LarsWiegers/laravel-ai-evaluation',
      },
    ],
    sidebar: [
      {
        text: 'Getting Started',
        items: [
          { text: 'First eval in 5 minutes', link: '/first-eval' },
          { text: 'Installation', link: '/installation' },
          { text: 'Concepts', link: '/concepts' },
          { text: 'Create eval files', link: '/creating-evals' },
          { text: 'Dataset evals', link: '/datasets' },
          { text: 'Conversation evals', link: '/conversation-evals' },
          { text: 'API reference', link: '/api-reference' },
          { text: 'Troubleshooting', link: '/troubleshooting' },
          { text: 'Dealing with rate limits', link: '/dealing-with-rate-limits' },
          { text: 'When to run evals', link: '/when-to-run-evals' },
          { text: 'Run in CI', link: '/running-in-ci' },
        ],
      },
      {
        text: 'Pest',
        items: [{ text: 'Run in Pest', link: '/running-in-pest' }],
      },
      {
        text: 'Standalone',
        items: [{ text: 'Run standalone', link: '/running-standalone' }],
      },
      {
        text: 'Expectations',
        items: [
          { text: 'Overview', link: '/expectations' },
          { text: 'Deterministic expectations', link: '/deterministic-expectations' },
          { text: 'LLM-as-judge expectations', link: '/llm-as-judge-expectations' },
          { text: 'Custom expectations', link: '/custom-expectations' },
          { text: 'Dataset evals', link: '/datasets' },
          { text: 'Conversation evals', link: '/conversation-evals' },
        ],
      },
    ],
  },
})
