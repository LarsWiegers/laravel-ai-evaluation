...
it('prompts through laravel ai anonymous agent fallback', function () {
    if (! class_exists('Laravel\\Ai\\AnonymousAgent')) {
        eval('namespace Laravel\\Ai; class AnonymousAgent { public function __construct(public string $instructions = "") {} public function prompt(string $prompt): string { return "stub:{$prompt}"; } }');
    }
